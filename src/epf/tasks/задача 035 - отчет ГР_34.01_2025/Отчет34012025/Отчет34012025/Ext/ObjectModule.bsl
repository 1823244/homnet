...
Функция СведенияОВнешнейОбработке() Экспорт
	...
	ПараметрыРегистрации.Вставить("Наименование","Отчет34012025");
	ПараметрыРегистрации.Вставить("БезопасныйРежим", Ложь);
	ПараметрыРегистрации.Вставить("Информация","Отчет 34.01_2025");
	ПараметрыРегистрации.Вставить("ВерсияБСП", "3.1.5.180");
	//ПараметрыРегистрации.Вставить("ОпределитьНастройкиФормы", Ложь);

	ТипКоманды = ДополнительныеОтчетыИОбработкиКлиентСервер.ТипКомандыОткрытиеФормы();
	ДобавитьКоманду(ПараметрыРегистрации.Команды,
		"Открыть форму : Отчет 34.01_2025",
		"Форма_Отчет34012025",
		ТипКоманды,
		Ложь) ;
...
// Назначение: Основная процедура запуска создания всех показателей отчета ГР_34.01_2025
...
Процедура СоздатьОбновитьПоказатели(Знач ВыводитьСообщения = Истина, Знач РежимТестирования = Ложь) Экспорт
	...
	ИмяГруппы = "ГР_34_01_2025";

	Если ВыводитьСообщения Тогда

		Если РежимТестирования Тогда
			Сообщить("<operation_start>Начало создания показателей отчета ГР_34.01_2025 (РЕЖИМ ТЕСТИРОВАНИЯ)</operation_start>");
		Иначе
			Сообщить("<operation_start>Начало создания показателей отчета ГР_34.01_2025</operation_start>");
		КонецЕсли;
	...
	Если ВыводитьСообщения Тогда
		Сообщить("<operation_end>Завершено создание показателей отчета ГР_34.01_2025</operation_end>");
	КонецЕсли;
...
Функция ТекстКодаДетальногоПоказателя(НомерСтроки, НомерКолонки, РежимТестирования = Ложь)
	...
	|	.НайтиПоНаименованию(""Отчет34012025"", Истина);
...
Процедура СоздатьДетальныеПоказателиИзТЗ(ТЗ, Знач РежимТестирования = Ложь, Знач ВыводитьСообщения = Ложь)
	...
	Для Счетчик = 0 По ТЗ.Количество() - 1 Цикл
		...
		Идентификатор = "ГР_34_01_2025_стр" + Формат(стрк.НомерСтроки, "ЧГ=;ЧРГ=") + "кол" + Формат(стрк.НомерКолонки, "ЧГ=;ЧРГ=");
...
Процедура Добавить_Группировочные_Показатели_в_ТЗ(ТЗ, Знач РежимТестирования = Ложь)
	...
	стрк.НомерКолонки = 3;
	стрк.ТипПоказателяОтчета = Перечисления.хн_нфо_ТипПоказателяОтчета.ОстатокНаКонец;
	стрк.СписокИспользуемыхПоказателей = "ГР_34_01_2025_стр2кол3,ГР_34_01_2025_стр3кол3,ГР_34_01_2025_стр4кол3,ГР_34_01_2025_стр5кол3,ГР_34_01_2025_стр6кол3,ГР_34_01_2025_стр7кол3,ГР_34_01_2025_стр8кол3";

	стрк = ТЗ.Добавить();
	стрк.НомерСтроки = 1;
	стрк.НомерКолонки = 4;
	стрк.ТипПоказателяОтчета = Перечисления.хн_нфо_ТипПоказателяОтчета.ОстатокНаКонец;
	стрк.СписокИспользуемыхПоказателей = "ГР_34_01_2025_стр2кол4,ГР_34_01_2025_стр3кол4,ГР_34_01_2025_стр4кол4,ГР_34_01_2025_стр5кол4,ГР_34_01_2025_стр6кол4,ГР_34_01_2025_стр7кол4,ГР_34_01_2025_стр8кол4";
	...
	стрк.НомерКолонки = 3;
	стрк.ТипПоказателяОтчета = Перечисления.хн_нфо_ТипПоказателяОтчета.ОстатокНаКонец;
	стрк.СписокИспользуемыхПоказателей = "ГР_34_01_2025_стр10кол3,ГР_34_01_2025_стр11кол3,ГР_34_01_2025_стр12кол3,ГР_34_01_2025_стр13кол3,ГР_34_01_2025_стр14кол3";

	стрк = ТЗ.Добавить();
	стрк.НомерСтроки = 9;
	стрк.НомерКолонки = 4;
	стрк.ТипПоказателяОтчета = Перечисления.хн_нфо_ТипПоказателяОтчета.ОстатокНаКонец;
	стрк.СписокИспользуемыхПоказателей = "ГР_34_01_2025_стр10кол4,ГР_34_01_2025_стр11кол4,ГР_34_01_2025_стр12кол4,ГР_34_01_2025_стр13кол4,ГР_34_01_2025_стр14кол4";
	...
	стрк.НомерКолонки = 3;
	стрк.ТипПоказателяОтчета = Перечисления.хн_нфо_ТипПоказателяОтчета.ОстатокНаКонец;
	стрк.СписокИспользуемыхПоказателей = "ГР_34_01_2025_стр1кол3,ГР_34_01_2025_стр9кол3";

	стрк = ТЗ.Добавить();
	стрк.НомерСтроки = 15;
	стрк.НомерКолонки = 4;
	стрк.ТипПоказателяОтчета = Перечисления.хн_нфо_ТипПоказателяОтчета.ОстатокНаКонец;
	стрк.СписокИспользуемыхПоказателей = "ГР_34_01_2025_стр1кол4,ГР_34_01_2025_стр9кол4";
...
Функция ВычислитьПоказатель_стр2кол3(Знач ПериодОстатков = Неопределено)
	...
	КодыСчетов.Добавить("71001");
	КодыСчетов.Добавить("71004");
	КодыСчетов.Добавить("71005");
	КодыСчетов.Добавить("71006");
	КодыСчетов.Добавить("72001");
	КодыСчетов.Добавить("72004");
	КодыСчетов.Добавить("72005");
	КодыСчетов.Добавить("72006");
	
	СимволыОФР=Новый ТаблицаЗначений;
	СимволыОФР.Колонки.Добавить("Код");
	СимволыОФР.Колонки.Добавить("Знак");     // не используется
	
	т=СимволыОФР.Добавить();
	т.код="31126";
	т.знак=1;
	т=СимволыОФР.Добавить();
	т.код="31129";
...
Функция ВычислитьПоказатель_стр6кол3(Знач ПериодОстатков = Неопределено)
	...
	КодыСчетов = новый Массив;
	КодыСчетов.Добавить("71001");
	КодыСчетов.Добавить("71002");
	КодыСчетов.Добавить("71003");
	КодыСчетов.Добавить("71004");
	КодыСчетов.Добавить("71005");
	КодыСчетов.Добавить("71006");
	КодыСчетов.Добавить("72001");
	КодыСчетов.Добавить("72002");
	КодыСчетов.Добавить("72003");
	КодыСчетов.Добавить("72004");
	КодыСчетов.Добавить("72005");
	КодыСчетов.Добавить("72006");
	...
	т=СимволыОФР.Добавить();т.код="31111";
	т=СимволыОФР.Добавить();т.код="31110";
	т=СимволыОФР.Добавить();т.код="31147";
	т=СимволыОФР.Добавить();т.код="31150";
	т=СимволыОФР.Добавить();т.код="31310";
	т=СимволыОФР.Добавить();т.код="31443";
	т=СимволыОФР.Добавить();т.код="31645";
	т=СимволыОФР.Добавить();т.код="31648";
