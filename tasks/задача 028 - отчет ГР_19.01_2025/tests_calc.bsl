...
#region test_execution
	...
	Для Каждого ИмяМетода Из МассивМетодов Цикл
		...
		Исключение
			Сообщить("<test_error>Ошибка в " + ИмяМетода + ": " + ОписаниеОшибки() + "</test_error>");
		КонецПопытки;
...
// СТРОКА 3 - Остатки на начало предыдущего периода
...
#region calculation
	ЗначениеПоказателя = ОстатокПоСчету(Справочники.Организации.ПустаяСсылка(), СчетСсылка, СубконтоСсылка, ДатаОстатка, ЭтоОстатокНаНачало);
	ЛогВычисления_Остатки(3,3,ЭтоОстатокНаНачало,СчетСсылка,ДатаОстатка,Новый Структура,СубконтоСсылка,ЗначениеПоказателя);
#endregion
	Возврат ЗначениеПоказателя;
КонецФункции
...
#region calculation
	ЗначениеПоказателя = ОстатокПоСчету(Справочники.Организации.ПустаяСсылка(), СчетСсылка, СубконтоСсылка, ДатаОстатка, ЭтоОстатокНаНачало);
	ЛогВычисления_Остатки(3,4,ЭтоОстатокНаНачало,СчетСсылка,ДатаОстатка,Новый Структура,СубконтоСсылка,ЗначениеПоказателя);
#endregion
	Возврат ЗначениеПоказателя;
КонецФункции
...
#region calculation
	ЗначениеПоказателя = ОстатокПоСчету(Справочники.Организации.ПустаяСсылка(), СчетСсылка, СубконтоСсылка, ДатаОстатка, ЭтоОстатокНаНачало);
	ЛогВычисления_Остатки(3,6,ЭтоОстатокНаНачало,СчетСсылка,ДатаОстатка,Новый Структура,СубконтоСсылка,ЗначениеПоказателя);
#endregion
	Возврат ЗначениеПоказателя;
КонецФункции
...
#region calculation
	ЗначениеПоказателя = ОстатокПоСчету(Справочники.Организации.ПустаяСсылка(), СчетСсылка, СубконтоСсылка, ДатаОстатка, ЭтоОстатокНаНачало);
	ЛогВычисления_Остатки(3,7,ЭтоОстатокНаНачало,СчетСсылка,ДатаОстатка,Новый Структура,СубконтоСсылка,ЗначениеПоказателя);
#endregion
	Возврат ЗначениеПоказателя;
КонецФункции

// СТРОКА 5 - Дебетовые обороты предыдущего периода

Функция ВычислитьПоказатель_стр5кол3()
#region initialization
	ЭтоДебетовыйОборот = Истина;
	СчетСсылка = ПланыСчетов.Хозрасчетный.НайтиПоКоду("51");
	_НачалоПериода = НачалоГода(НачалоГода(Дата(2024, 1, 1))-1);
	_КонецПериода = КонецПредыдущегоПериода();
	СубконтоСсылка = ГруппаОС_Сооружения();
#endregion
#region calculation
	ЗначениеПоказателя = ОборотыПоСчету(Справочники.Организации.ПустаяСсылка(), СчетСсылка, СубконтоСсылка, _НачалоПериода, _КонецПериода, ЭтоДебетовыйОборот);
	ЛогВычисления_Обороты(5,3,ЭтоДебетовыйОборот,СчетСсылка,_НачалоПериода,_КонецПериода,Новый Структура,СубконтоСсылка,ЗначениеПоказателя);
#endregion
	Возврат ЗначениеПоказателя;
КонецФункции
...
#region calculation
	ЗначениеПоказателя = ОборотыПоСчету(Справочники.Организации.ПустаяСсылка(), СчетСсылка, СубконтоСсылка, _НачалоПериода, _КонецПериода, ЭтоДебетовыйОборот);
	ЛогВычисления_Обороты(5,4,ЭтоДебетовыйОборот,СчетСсылка,_НачалоПериода,_КонецПериода,Новый Структура,СубконтоСсылка,ЗначениеПоказателя);
#endregion
	Возврат ЗначениеПоказателя;
КонецФункции
...
#region calculation
	ЗначениеПоказателя = ОборотыПоСчету(Справочники.Организации.ПустаяСсылка(), СчетСсылка, СубконтоСсылка, _НачалоПериода, _КонецПериода, ЭтоДебетовыйОборот);
	ЛогВычисления_Обороты(5,6,ЭтоДебетовыйОборот,СчетСсылка,_НачалоПериода,_КонецПериода,Новый Структура,СубконтоСсылка,ЗначениеПоказателя);
#endregion
	Возврат ЗначениеПоказателя;
КонецФункции
...
#region calculation
	ЗначениеПоказателя = ОборотыПоСчету(Справочники.Организации.ПустаяСсылка(), СчетСсылка, СубконтоСсылка, _НачалоПериода, _КонецПериода, ЭтоДебетовыйОборот);
	ЛогВычисления_Обороты(5,7,ЭтоДебетовыйОборот,СчетСсылка,_НачалоПериода,_КонецПериода,Новый Структура,СубконтоСсылка,ЗначениеПоказателя);
#endregion
	Возврат ЗначениеПоказателя;
КонецФункции

// СТРОКА 10 - Кредитовые обороты предыдущего периода
...
#region calculation
	ЗначениеПоказателя = ОборотыПоСчету(Справочники.Организации.ПустаяСсылка(), СчетСсылка, СубконтоСсылка, _НачалоПериода, _КонецПериода, ЭтоДебетовыйОборот);
	ЛогВычисления_Обороты(10,3,ЭтоДебетовыйОборот,СчетСсылка,_НачалоПериода,_КонецПериода,Новый Структура,СубконтоСсылка,ЗначениеПоказателя);
#endregion
	Возврат ЗначениеПоказателя;
КонецФункции
...
#region calculation
	ЗначениеПоказателя = ОборотыПоСчету(Справочники.Организации.ПустаяСсылка(), СчетСсылка, СубконтоСсылка, _НачалоПериода, _КонецПериода, ЭтоДебетовыйОборот);
	ЛогВычисления_Обороты(10,4,ЭтоДебетовыйОборот,СчетСсылка,_НачалоПериода,_КонецПериода,Новый Структура,СубконтоСсылка,ЗначениеПоказателя);
#endregion
	Возврат ЗначениеПоказателя;
КонецФункции
...
#region calculation
	ЗначениеПоказателя = ОборотыПоСчету(Справочники.Организации.ПустаяСсылка(), СчетСсылка, СубконтоСсылка, _НачалоПериода, _КонецПериода, ЭтоДебетовыйОборот);
	ЛогВычисления_Обороты(10,6,ЭтоДебетовыйОборот,СчетСсылка,_НачалоПериода,_КонецПериода,Новый Структура,СубконтоСсылка,ЗначениеПоказателя);
#endregion
	Возврат ЗначениеПоказателя;
КонецФункции
...
#region calculation
	ЗначениеПоказателя = ОборотыПоСчету(Справочники.Организации.ПустаяСсылка(), СчетСсылка, СубконтоСсылка, _НачалоПериода, _КонецПериода, ЭтоДебетовыйОборот);
	ЛогВычисления_Обороты(10,7,ЭтоДебетовыйОборот,СчетСсылка,_НачалоПериода,_КонецПериода,Новый Структура,СубконтоСсылка,ЗначениеПоказателя);
#endregion
	Возврат ЗначениеПоказателя;
КонецФункции

// СТРОКА 11 - Дебетовые обороты предыдущего периода (дублирует строку 5)
...
#region calculation
	ЗначениеПоказателя = ОборотыПоСчету(Справочники.Организации.ПустаяСсылка(), СчетСсылка, СубконтоСсылка, _НачалоПериода, _КонецПериода, ЭтоДебетовыйОборот);
	ЛогВычисления_Обороты(11,3,ЭтоДебетовыйОборот,СчетСсылка,_НачалоПериода,_КонецПериода,Новый Структура,СубконтоСсылка,ЗначениеПоказателя);
#endregion
	Возврат ЗначениеПоказателя;
КонецФункции
...
#region calculation
	ЗначениеПоказателя = ОборотыПоСчету(Справочники.Организации.ПустаяСсылка(), СчетСсылка, СубконтоСсылка, _НачалоПериода, _КонецПериода, ЭтоДебетовыйОборот);
	ЛогВычисления_Обороты(11,4,ЭтоДебетовыйОборот,СчетСсылка,_НачалоПериода,_КонецПериода,Новый Структура,СубконтоСсылка,ЗначениеПоказателя);
#endregion
	Возврат ЗначениеПоказателя;
КонецФункции
...
#region calculation
	ЗначениеПоказателя = ОборотыПоСчету(Справочники.Организации.ПустаяСсылка(), СчетСсылка, СубконтоСсылка, _НачалоПериода, _КонецПериода, ЭтоДебетовыйОборот);
	ЛогВычисления_Обороты(11,6,ЭтоДебетовыйОборот,СчетСсылка,_НачалоПериода,_КонецПериода,Новый Структура,СубконтоСсылка,ЗначениеПоказателя);
#endregion
	Возврат ЗначениеПоказателя;
КонецФункции
...
#region calculation
	ЗначениеПоказателя = ОборотыПоСчету(Справочники.Организации.ПустаяСсылка(), СчетСсылка, СубконтоСсылка, _НачалоПериода, _КонецПериода, ЭтоДебетовыйОборот);
	ЛогВычисления_Обороты(11,7,ЭтоДебетовыйОборот,СчетСсылка,_НачалоПериода,_КонецПериода,Новый Структура,СубконтоСсылка,ЗначениеПоказателя);
#endregion
	Возврат ЗначениеПоказателя;
КонецФункции

// СТРОКА 13 - Кредитовые обороты предыдущего периода (дублирует строку 10)
...
#region calculation
	ЗначениеПоказателя = ОборотыПоСчету(Справочники.Организации.ПустаяСсылка(), СчетСсылка, СубконтоСсылка, _НачалоПериода, _КонецПериода, ЭтоДебетовыйОборот);
	ЛогВычисления_Обороты(13,3,ЭтоДебетовыйОборот,СчетСсылка,_НачалоПериода,_КонецПериода,Новый Структура,СубконтоСсылка,ЗначениеПоказателя);
#endregion
	Возврат ЗначениеПоказателя;
КонецФункции
...
#region calculation
	ЗначениеПоказателя = ОборотыПоСчету(Справочники.Организации.ПустаяСсылка(), СчетСсылка, СубконтоСсылка, _НачалоПериода, _КонецПериода, ЭтоДебетовыйОборот);
	ЛогВычисления_Обороты(13,4,ЭтоДебетовыйОборот,СчетСсылка,_НачалоПериода,_КонецПериода,Новый Структура,СубконтоСсылка,ЗначениеПоказателя);
#endregion
	Возврат ЗначениеПоказателя;
КонецФункции
...
#region calculation
	ЗначениеПоказателя = ОборотыПоСчету(Справочники.Организации.ПустаяСсылка(), СчетСсылка, СубконтоСсылка, _НачалоПериода, _КонецПериода, ЭтоДебетовыйОборот);
	ЛогВычисления_Обороты(13,6,ЭтоДебетовыйОборот,СчетСсылка,_НачалоПериода,_КонецПериода,Новый Структура,СубконтоСсылка,ЗначениеПоказателя);
#endregion
	Возврат ЗначениеПоказателя;
КонецФункции
...
#region calculation
	ЗначениеПоказателя = ОборотыПоСчету(Справочники.Организации.ПустаяСсылка(), СчетСсылка, СубконтоСсылка, _НачалоПериода, _КонецПериода, ЭтоДебетовыйОборот);
	ЛогВычисления_Обороты(13,7,ЭтоДебетовыйОборот,СчетСсылка,_НачалоПериода,_КонецПериода,Новый Структура,СубконтоСсылка,ЗначениеПоказателя);
#endregion
	Возврат ЗначениеПоказателя;
КонецФункции

// СТРОКА 25 - Остатки на конец предыдущего периода
...
#region calculation
	ЗначениеПоказателя = ОстатокПоСчету(Справочники.Организации.ПустаяСсылка(), СчетСсылка, СубконтоСсылка, ДатаОстатка, ЭтоОстатокНаНачало);
	ЛогВычисления_Остатки(25,3,ЭтоОстатокНаНачало,СчетСсылка,ДатаОстатка,Новый Структура,СубконтоСсылка,ЗначениеПоказателя);
#endregion
	Возврат ЗначениеПоказателя;
КонецФункции
...
#region calculation
	ЗначениеПоказателя = ОстатокПоСчету(Справочники.Организации.ПустаяСсылка(), СчетСсылка, СубконтоСсылка, ДатаОстатка, ЭтоОстатокНаНачало);
	ЛогВычисления_Остатки(25,4,ЭтоОстатокНаНачало,СчетСсылка,ДатаОстатка,Новый Структура,СубконтоСсылка,ЗначениеПоказателя);
#endregion
	Возврат ЗначениеПоказателя;
КонецФункции
...
#region calculation
	ЗначениеПоказателя = ОстатокПоСчету(Справочники.Организации.ПустаяСсылка(), СчетСсылка, СубконтоСсылка, ДатаОстатка, ЭтоОстатокНаНачало);
	ЛогВычисления_Остатки(25,6,ЭтоОстатокНаНачало,СчетСсылка,ДатаОстатка,Новый Структура,СубконтоСсылка,ЗначениеПоказателя);
#endregion
	Возврат ЗначениеПоказателя;
КонецФункции
...
#region calculation
	ЗначениеПоказателя = ОстатокПоСчету(Справочники.Организации.ПустаяСсылка(), СчетСсылка, СубконтоСсылка, ДатаОстатка, ЭтоОстатокНаНачало);
	ЛогВычисления_Остатки(25,7,ЭтоОстатокНаНачало,СчетСсылка,ДатаОстатка,Новый Структура,СубконтоСсылка,ЗначениеПоказателя);
#endregion
	Возврат ЗначениеПоказателя;
КонецФункции

// СТРОКА 26 - Остатки на конец предыдущего периода (дублирует строку 25)
...
#region calculation
	ЗначениеПоказателя = ОстатокПоСчету(Справочники.Организации.ПустаяСсылка(), СчетСсылка, СубконтоСсылка, ДатаОстатка, ЭтоОстатокНаНачало);
	ЛогВычисления_Остатки(26,3,ЭтоОстатокНаНачало,СчетСсылка,ДатаОстатка,Новый Структура,СубконтоСсылка,ЗначениеПоказателя);
#endregion
	Возврат ЗначениеПоказателя;
КонецФункции
...
#region calculation
	ЗначениеПоказателя = ОстатокПоСчету(Справочники.Организации.ПустаяСсылка(), СчетСсылка, СубконтоСсылка, ДатаОстатка, ЭтоОстатокНаНачало);
	ЛогВычисления_Остатки(26,4,ЭтоОстатокНаНачало,СчетСсылка,ДатаОстатка,Новый Структура,СубконтоСсылка,ЗначениеПоказателя);
#endregion
	Возврат ЗначениеПоказателя;
КонецФункции
...
#region calculation
	ЗначениеПоказателя = ОстатокПоСчету(Справочники.Организации.ПустаяСсылка(), СчетСсылка, СубконтоСсылка, ДатаОстатка, ЭтоОстатокНаНачало);
	ЛогВычисления_Остатки(26,6,ЭтоОстатокНаНачало,СчетСсылка,ДатаОстатка,Новый Структура,СубконтоСсылка,ЗначениеПоказателя);
#endregion
	Возврат ЗначениеПоказателя;
КонецФункции
...
#region calculation
	ЗначениеПоказателя = ОстатокПоСчету(Справочники.Организации.ПустаяСсылка(), СчетСсылка, СубконтоСсылка, ДатаОстатка, ЭтоОстатокНаНачало);
	ЛогВычисления_Остатки(26,7,ЭтоОстатокНаНачало,СчетСсылка,ДатаОстатка,Новый Структура,СубконтоСсылка,ЗначениеПоказателя);
#endregion
	Возврат ЗначениеПоказателя;
КонецФункции
...
// СТРОКА 29 - Остатки на начало текущего периода

Функция ВычислитьПоказатель_стр29кол3()
#region initialization
	ЭтоОстатокНаНачало = Истина;
	СчетСсылка = ПланыСчетов.Хозрасчетный.НайтиПоКоду("51");
	ДатаОстатка = НачалоГода(Дата(2024, 1, 1));
	СубконтоСсылка = ГруппаОС_Сооружения();
#endregion
#region calculation
	ЗначениеПоказателя = ОстатокПоСчету(Справочники.Организации.ПустаяСсылка(), СчетСсылка, СубконтоСсылка, ДатаОстатка, ЭтоОстатокНаНачало);
	ЛогВычисления_Остатки(29,3,ЭтоОстатокНаНачало,СчетСсылка,ДатаОстатка,Новый Структура,СубконтоСсылка,ЗначениеПоказателя);
#endregion
	Возврат ЗначениеПоказателя;
КонецФункции
...
#region calculation
	ЗначениеПоказателя = ОстатокПоСчету(Справочники.Организации.ПустаяСсылка(), СчетСсылка, СубконтоСсылка, ДатаОстатка, ЭтоОстатокНаНачало);
	ЛогВычисления_Остатки(29,4,ЭтоОстатокНаНачало,СчетСсылка,ДатаОстатка,Новый Структура,СубконтоСсылка,ЗначениеПоказателя);
#endregion
	Возврат ЗначениеПоказателя;
КонецФункции
...
#region calculation
	ЗначениеПоказателя = ОстатокПоСчету(Справочники.Организации.ПустаяСсылка(), СчетСсылка, СубконтоСсылка, ДатаОстатка, ЭтоОстатокНаНачало);
	ЛогВычисления_Остатки(29,6,ЭтоОстатокНаНачало,СчетСсылка,ДатаОстатка,Новый Структура,СубконтоСсылка,ЗначениеПоказателя);
#endregion
	Возврат ЗначениеПоказателя;
КонецФункции
...
#region calculation
	ЗначениеПоказателя = ОстатокПоСчету(Справочники.Организации.ПустаяСсылка(), СчетСсылка, СубконтоСсылка, ДатаОстатка, ЭтоОстатокНаНачало);
	ЛогВычисления_Остатки(29,7,ЭтоОстатокНаНачало,СчетСсылка,ДатаОстатка,Новый Структура,СубконтоСсылка,ЗначениеПоказателя);
#endregion
	Возврат ЗначениеПоказателя;
КонецФункции

// СТРОКА 30 - Остатки на начало текущего периода (дублирует строку 29)
...
#region calculation
	ЗначениеПоказателя = ОстатокПоСчету(Справочники.Организации.ПустаяСсылка(), СчетСсылка, СубконтоСсылка, ДатаОстатка, ЭтоОстатокНаНачало);
	ЛогВычисления_Остатки(30,3,ЭтоОстатокНаНачало,СчетСсылка,ДатаОстатка,Новый Структура,СубконтоСсылка,ЗначениеПоказателя);
#endregion
	Возврат ЗначениеПоказателя;
КонецФункции
...
#region calculation
	ЗначениеПоказателя = ОстатокПоСчету(Справочники.Организации.ПустаяСсылка(), СчетСсылка, СубконтоСсылка, ДатаОстатка, ЭтоОстатокНаНачало);
	ЛогВычисления_Остатки(30,4,ЭтоОстатокНаНачало,СчетСсылка,ДатаОстатка,Новый Структура,СубконтоСсылка,ЗначениеПоказателя);
#endregion
	Возврат ЗначениеПоказателя;
КонецФункции
...
#region calculation
	ЗначениеПоказателя = ОстатокПоСчету(Справочники.Организации.ПустаяСсылка(), СчетСсылка, СубконтоСсылка, ДатаОстатка, ЭтоОстатокНаНачало);
	ЛогВычисления_Остатки(30,6,ЭтоОстатокНаНачало,СчетСсылка,ДатаОстатка,Новый Структура,СубконтоСсылка,ЗначениеПоказателя);
#endregion
	Возврат ЗначениеПоказателя;
КонецФункции
...
#region calculation
	ЗначениеПоказателя = ОстатокПоСчету(Справочники.Организации.ПустаяСсылка(), СчетСсылка, СубконтоСсылка, ДатаОстатка, ЭтоОстатокНаНачало);
	ЛогВычисления_Остатки(30,7,ЭтоОстатокНаНачало,СчетСсылка,ДатаОстатка,Новый Структура,СубконтоСсылка,ЗначениеПоказателя);
#endregion
	Возврат ЗначениеПоказателя;
КонецФункции

// СТРОКА 32 - Дебетовые обороты текущего периода

Функция ВычислитьПоказатель_стр32кол3()
#region initialization
	ЭтоДебетовыйОборот = Истина;
	СчетСсылка = ПланыСчетов.Хозрасчетный.НайтиПоКоду("51");
	_НачалоПериода = НачалоГода(Дата(2024, 1, 1));
	_КонецПериода = Дата(2024, 12, 31);
	СубконтоСсылка = ГруппаОС_Сооружения();
#endregion
#region calculation
	ЗначениеПоказателя = ОборотыПоСчету(Справочники.Организации.ПустаяСсылка(), СчетСсылка, СубконтоСсылка, _НачалоПериода, _КонецПериода, ЭтоДебетовыйОборот);
	ЛогВычисления_Обороты(32,3,ЭтоДебетовыйОборот,СчетСсылка,_НачалоПериода,_КонецПериода,Новый Структура,СубконтоСсылка,ЗначениеПоказателя);
#endregion
	Возврат ЗначениеПоказателя;
КонецФункции
...
#region calculation
	ЗначениеПоказателя = ОборотыПоСчету(Справочники.Организации.ПустаяСсылка(), СчетСсылка, СубконтоСсылка, _НачалоПериода, _КонецПериода, ЭтоДебетовыйОборот);
	ЛогВычисления_Обороты(32,4,ЭтоДебетовыйОборот,СчетСсылка,_НачалоПериода,_КонецПериода,Новый Структура,СубконтоСсылка,ЗначениеПоказателя);
#endregion
	Возврат ЗначениеПоказателя;
КонецФункции
...
#region calculation
	ЗначениеПоказателя = ОборотыПоСчету(Справочники.Организации.ПустаяСсылка(), СчетСсылка, СубконтоСсылка, _НачалоПериода, _КонецПериода, ЭтоДебетовыйОборот);
	ЛогВычисления_Обороты(32,6,ЭтоДебетовыйОборот,СчетСсылка,_НачалоПериода,_КонецПериода,Новый Структура,СубконтоСсылка,ЗначениеПоказателя);
#endregion
	Возврат ЗначениеПоказателя;
КонецФункции
...
#region calculation
	ЗначениеПоказателя = ОборотыПоСчету(Справочники.Организации.ПустаяСсылка(), СчетСсылка, СубконтоСсылка, _НачалоПериода, _КонецПериода, ЭтоДебетовыйОборот);
	ЛогВычисления_Обороты(32,7,ЭтоДебетовыйОборот,СчетСсылка,_НачалоПериода,_КонецПериода,Новый Структура,СубконтоСсылка,ЗначениеПоказателя);
#endregion
	Возврат ЗначениеПоказателя;
КонецФункции

// СТРОКА 37 - Кредитовые обороты текущего периода
...
Функция ВычислитьПоказатель_стр37кол3()
	...
	СубконтоСсылка = ГруппаОС_Сооружения();
	ЗначениеПоказателя = ОборотыПоСчету(Справочники.Организации.ПустаяСсылка(), СчетСсылка, СубконтоСсылка, _НачалоПериода, _КонецПериода, ЭтоДебетовыйОборот);
	ЛогВычисления_Обороты(37,3,ЭтоДебетовыйОборот,СчетСсылка,_НачалоПериода,_КонецПериода,Новый Структура,СубконтоСсылка,ЗначениеПоказателя);
	Возврат ЗначениеПоказателя;
КонецФункции

Функция ВычислитьПоказатель_стр37кол4()
	ЭтоДебетовыйОборот = Ложь; СчетСсылка = ПланыСчетов.Хозрасчетный.НайтиПоКоду("51"); _НачалоПериода = НачалоГода(Дата(2024, 1, 1)); _КонецПериода = Дата(2024, 12, 31); СубконтоСсылка = ГруппаОС_ОфисноеОборудование();
	ЗначениеПоказателя = ОборотыПоСчету(Справочники.Организации.ПустаяСсылка(), СчетСсылка, СубконтоСсылка, _НачалоПериода, _КонецПериода, ЭтоДебетовыйОборот);
	ЛогВычисления_Обороты(37,4,ЭтоДебетовыйОборот,СчетСсылка,_НачалоПериода,_КонецПериода,Новый Структура,СубконтоСсылка,ЗначениеПоказателя); Возврат ЗначениеПоказателя; КонецФункции

Функция ВычислитьПоказатель_стр37кол6()
	ЭтоДебетовыйОборот = Ложь; СчетСсылка = ПланыСчетов.Хозрасчетный.НайтиПоКоду("51"); _НачалоПериода = НачалоГода(Дата(2024, 1, 1)); _КонецПериода = Дата(2024, 12, 31); СубконтоСсылка = ГруппаОС_Другие();
	ЗначениеПоказателя = ОборотыПоСчету(Справочники.Организации.ПустаяСсылка(), СчетСсылка, СубконтоСсылка, _НачалоПериода, _КонецПериода, ЭтоДебетовыйОборот);
	ЛогВычисления_Обороты(37,6,ЭтоДебетовыйОборот,СчетСсылка,_НачалоПериода,_КонецПериода,Новый Структура,СубконтоСсылка,ЗначениеПоказателя); Возврат ЗначениеПоказателя; КонецФункции

Функция ВычислитьПоказатель_стр37кол7()
	ЭтоДебетовыйОборот = Ложь; СчетСсылка = ПланыСчетов.Хозрасчетный.НайтиПоКоду("51"); _НачалоПериода = НачалоГода(Дата(2024, 1, 1)); _КонецПериода = Дата(2024, 12, 31); СубконтоСсылка = Неопределено;
	ЗначениеПоказателя = ОборотыПоСчету(Справочники.Организации.ПустаяСсылка(), СчетСсылка, СубконтоСсылка, _НачалоПериода, _КонецПериода, ЭтоДебетовыйОборот);
	ЛогВычисления_Обороты(37,7,ЭтоДебетовыйОборот,СчетСсылка,_НачалоПериода,_КонецПериода,Новый Структура,СубконтоСсылка,ЗначениеПоказателя); Возврат ЗначениеПоказателя; КонецФункции

// СТРОКА 38 - Дебетовые обороты текущего периода
Функция ВычислитьПоказатель_стр38кол3()
	ЭтоДебетовыйОборот = Истина; СчетСсылка = ПланыСчетов.Хозрасчетный.НайтиПоКоду("51"); _НачалоПериода = НачалоГода(Дата(2024, 1, 1)); _КонецПериода = Дата(2024, 12, 31); СубконтоСсылка = ГруппаОС_Сооружения();
	ЗначениеПоказателя = ОборотыПоСчету(Справочники.Организации.ПустаяСсылка(), СчетСсылка, СубконтоСсылка, _НачалоПериода, _КонецПериода, ЭтоДебетовыйОборот);
	ЛогВычисления_Обороты(38,3,ЭтоДебетовыйОборот,СчетСсылка,_НачалоПериода,_КонецПериода,Новый Структура,СубконтоСсылка,ЗначениеПоказателя); Возврат ЗначениеПоказателя; КонецФункции
Функция ВычислитьПоказатель_стр38кол4()
	ЭтоДебетовыйОборот = Истина; СчетСсылка = ПланыСчетов.Хозрасчетный.НайтиПоКоду("51"); _НачалоПериода = НачалоГода(Дата(2024, 1, 1)); _КонецПериода = Дата(2024, 12, 31); СубконтоСсылка = ГруппаОС_ОфисноеОборудование();
	ЗначениеПоказателя = ОборотыПоСчету(Справочники.Организации.ПустаяСсылка(), СчетСсылка, СубконтоСсылка, _НачалоПериода, _КонецПериода, ЭтоДебетовыйОборот);
	ЛогВычисления_Обороты(38,4,ЭтоДебетовыйОборот,СчетСсылка,_НачалоПериода,_КонецПериода,Новый Структура,СубконтоСсылка,ЗначениеПоказателя); Возврат ЗначениеПоказателя; КонецФункции
Функция ВычислитьПоказатель_стр38кол6()
	ЭтоДебетовыйОборот = Истина; СчетСсылка = ПланыСчетов.Хозрасчетный.НайтиПоКоду("51"); _НачалоПериода = НачалоГода(Дата(2024, 1, 1)); _КонецПериода = Дата(2024, 12, 31); СубконтоСсылка = ГруппаОС_Другие();
	ЗначениеПоказателя = ОборотыПоСчету(Справочники.Организации.ПустаяСсылка(), СчетСсылка, СубконтоСсылка, _НачалоПериода, _КонецПериода, ЭтоДебетовыйОборот);
	ЛогВычисления_Обороты(38,6,ЭтоДебетовыйОборот,СчетСсылка,_НачалоПериода,_КонецПериода,Новый Структура,СубконтоСсылка,ЗначениеПоказателя); Возврат ЗначениеПоказателя; КонецФункции
Функция ВычислитьПоказатель_стр38кол7()
	ЭтоДебетовыйОборот = Истина; СчетСсылка = ПланыСчетов.Хозрасчетный.НайтиПоКоду("51"); _НачалоПериода = НачалоГода(Дата(2024, 1, 1)); _КонецПериода = Дата(2024, 12, 31); СубконтоСсылка = Неопределено;
	ЗначениеПоказателя = ОборотыПоСчету(Справочники.Организации.ПустаяСсылка(), СчетСсылка, СубконтоСсылка, _НачалоПериода, _КонецПериода, ЭтоДебетовыйОборот);
	ЛогВычисления_Обороты(38,7,ЭтоДебетовыйОборот,СчетСсылка,_НачалоПериода,_КонецПериода,Новый Структура,СубконтоСсылка,ЗначениеПоказателя); Возврат ЗначениеПоказателя; КонецФункции

// Остается добавить: строки 40, 52, 53
