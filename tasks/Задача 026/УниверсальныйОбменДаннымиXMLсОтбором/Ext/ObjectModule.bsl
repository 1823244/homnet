...
Функция ОткрытьФайлВыгрузки(СтрокаСообщенияОбОшибке = "")
	...
	//СУУ_ЕНС 16 10 17 из-за даты выгрузки win merge думает, что все файлы изменены. придется отключить.
	УстановитьАтрибут(ВременныйЗаписьXML, "ДатаВыгрузки",				Неопределено);//ТекущаяДатаСеанса());
