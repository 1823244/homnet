...
Функция ТекстКодаДетальногоПоказателя(НомерСтроки, НомерКолонки, РежимТестирования = Ложь)
	...
	Если НомерКолонки = 3 Тогда
		// Колонка 3 - на конец периода
		ФормулаДаты = "КонецДня(мКонецПериода)";
	ИначеЕсли НомерКолонки = 4 Тогда
		// Колонка 4 - на 31 декабря предыдущего года
	...
	|	.НайтиПоНаименованию(""Отчет26012025"", Истина);
...
Процедура ДобавитьДетальныеПоказателиВТЗ(ТЗ, Знач РежимТестирования = Ложь)
	...
	// Колонка 4 - 31 декабря предыдущего года
...
// Списки хранятся в атрибуте ОписаниеПоказателя в формате "ИНН;КПП;Наименование" (каждая строка - один контрагент)
...
// Примечание: Список хранится в атрибуте ОписаниеПоказателя в виде многострочного текста
Процедура СохранитьСписокКонтрагентовВСправочник(ИдентификаторСписка, НаименованиеСписка, ТекстСписка) Экспорт
	...
	// Сохраняем список контрагентов в атрибут ОписаниеПоказателя
	// Формат: каждая строка - "ИНН;КПП;Наименование"
	ЭлементОбъект.ОписаниеПоказателя = ТекстСписка;
	
	ЭлементОбъект.ТекстКода = "ЗначениеПоказателя=0;";
...
// Примечание: Читает список из атрибута ОписаниеПоказателя и ищет контрагентов по ИНН+КПП
Функция ПолучитьСписокКонтрагентовИзСправочника(ИдентификаторСписка) Экспорт
	...
	// Читаем текст списка из атрибута ОписаниеПоказателя
	ТекстСписка = ЭлементСсылка.ОписаниеПоказателя;
	...
	Для Каждого СтрокаСписка Из МассивСтрок Цикл
		...
		Если МассивЧастей.Количество() >= 2 Тогда
			...
			Если НЕ ПустаяСтрока(ИНН) Тогда
				...
				Если ИНН = "3906328893" Тогда
					
					КонтрагентСсылка = Справочники.Контрагенты.НайтиПоКоду("18-000161");
					Если ЗначениеЗаполнено(КонтрагентСсылка) Тогда
						МассивКонтрагентов.Добавить(КонтрагентСсылка);
					КонецЕсли;
					КонтрагентСсылка = Справочники.Контрагенты.НайтиПоКоду("41033");
					Если ЗначениеЗаполнено(КонтрагентСсылка) Тогда
						МассивКонтрагентов.Добавить(КонтрагентСсылка);
					КонецЕсли;
...
// Формат хранения: в атрибуте ОписаниеПоказателя каждая строка - "ИНН;КПП;Наименование"
Процедура ИнициализироватьСпискиКонтрагентовВСправочнике() Экспорт
	...
	ТекстСписка =
	"7716212993;772601001;АБИСОФТ ООО
	|7709369584;772201001;АБИУС ООО
	|7704773521;772901001;АЙТИ ЛАЙТ ООО
	|7802544551;780201001;АКРИБИЯ. ПРОЕКТЫ И СЕРВИС ООО
	|7728450760;773101001;ВИНВЕСТОР АО
	|7713076301;771301001;ВЫМПЕЛКОМ ПАО
	|7702029405;770201001;МАКОМНЕТ АО
	|7710436796;502401001;МАСТЕРТЕЛ АО
	|7703465919;770301001;МБ ЗАЩИТА ИНФОРМАЦИИ ООО
	|7812014560;770701001;МЕГАФОН ПАО
	|7702077840;770301001;ПАО МОСКОВСКАЯ БИРЖА
	|6165189197;616501001;РНД СОФТ ООО
	|7707049388;784201001;РОСТЕЛЕКОМ ПАО
	|7707049388;616743001;РОСТЕЛЕКОМ ПАО
	|9722006181;772201001;СИНТЕГС ООО
	|9717050989;771701001;СКРИН МАРКЕТ СИСТЕМЗ ООО
	|7714970871;771401001;НФО АЙТИ ООО
	|7736643155;773601001;ТУРБО ТЕХНОЛОГИИ ООО";
	СохранитьСписокКонтрагентовВСправочник("Стр1_ИТУслуги", "Контрагенты по ИТ-услугам", ТекстСписка);
	
	// Строка 2 - Аренда помещений
	ТекстСписка =
	"7708234873;772401001;АПОЛЛО КОНСТРАКШН ООО
	|7704770513;770501001;ВАЛОВАЯ 37 ООО
	|3906328893;770501001;УК ЛЕРТА КАПИТАЛ ООО";
	...
	ТекстСписка =
	"7702165310;770101001;НКО АО НРД
	|7744000302;770401001;РАЙФФАЙЗЕНБАНК АО
...
Функция ВычислитьПоказатель_стр1кол4()
	...
	Для каждого Контрагент Из СубконтоСсылка Цикл
		ПараметрКонтрагент = Новый Массив;
		ПараметрКонтрагент.Добавить(Контрагент);
		_ЗначениеПоказателя = ОстатокПоСчету(мСтруктураПараметров.Организация, СчетСсылка, ПараметрКонтрагент, ДатаОстатка, ЭтоОстатокНаНачало);
		ЗначениеПоказателя = ЗначениеПоказателя + _ЗначениеПоказателя;
		сообщить("строка 1, кол. 4. Для контрагента "+строка(Контрагент)+" определен остаток: "+строка(_ЗначениеПоказателя));
	КонецЦикла;
