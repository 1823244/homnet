...
Функция ТекстКодаДетальногоПоказателя(НомерСтроки, НомерКолонки, РежимТестирования = Ложь)
	...
	#region date_formula_determination
	...
	ИначеЕсли НомерКолонки = 4 Тогда
		...
		ФормулаДаты = "НачалоГода(мНачалоПериода) - 1";
...
Процедура ДобавитьДетальныеПоказателиВТЗ(ТЗ, Знач РежимТестирования = Ложь)
	...
	// Строка 2: Заемные деньги
	// Колонка 3 - отчетная дата текущего года
	стрк = ТЗ.Добавить();
	стрк.НомерСтроки = 2;
	стрк.НомерКолонки = 3;
	стрк.ТипПоказателяОтчета = Перечисления.хн_нфо_ТипПоказателяОтчета.ОстатокНаКонец;

	// Колонка 4 - на начало года (31 декабря предыдущего года)
	стрк = ТЗ.Добавить();
	стрк.НомерСтроки = 2;
	стрк.НомерКолонки = 4;
	стрк.ТипПоказателяОтчета = Перечисления.хн_нфо_ТипПоказателяОтчета.ОстатокНаКонец;
	...
	#region placeholder_indicators

	// ВАЖНО: Остальные строки (3-7, 9-12) требуют уточнения счетов и логики вычисления
	// Пока добавляем их как заглушки для создания полной структуры показателей

	// Строка 3: Просрочка заемных денег
	стрк = ТЗ.Добавить();
	стрк.НомерСтроки = 3;
	стрк.НомерКолонки = 3;
	стрк.ТипПоказателяОтчета = Перечисления.хн_нфо_ТипПоказателяОтчета.ОстатокНаКонец;
	...
	// Строка 4: Прочие валютные обязательства
	стрк = ТЗ.Добавить();
	стрк.НомерСтроки = 4;
	стрк.НомерКолонки = 3;
	стрк.ТипПоказателяОтчета = Перечисления.хн_нфо_ТипПоказателяОтчета.ОстатокНаКонец;
	...
	// Строка 5: Просрочка обязательств, по которым наступает обязательство продажи валют
	стрк = ТЗ.Добавить();
	стрк.НомерСтроки = 5;
	стрк.НомерКолонки = 3;
	стрк.ТипПоказателяОтчета = Перечисления.хн_нфо_ТипПоказателяОтчета.ОстатокНаКонец;
	...
	// Строка 6: Выпуск облигаций и сертификатов с номиналом в иностранной валюте-брутто
	стрк = ТЗ.Добавить();
	стрк.НомерСтроки = 6;
	стрк.НомерКолонки = 3;
	стрк.ТипПоказателяОтчета = Перечисления.хн_нфо_ТипПоказателяОтчета.ОстатокНаКонец;
	...
	// Строка 9: Просрочка обязательств, по которым наступает обязательство продажи валют
	стрк = ТЗ.Добавить();
	стрк.НомерСтроки = 9;
	стрк.НомерКолонки = 3;
	стрк.ТипПоказателяОтчета = Перечисления.хн_нфо_ТипПоказателяОтчета.ОстатокНаКонец;
	...
	// Строка 10: Обязательства по операциям получения права пользования имуществом
	стрк = ТЗ.Добавить();
	стрк.НомерСтроки = 10;
	стрк.НомерКолонки = 3;
	стрк.ТипПоказателяОтчета = Перечисления.хн_нфо_ТипПоказателяОтчета.ОстатокНаКонец;
	...
	// Строка 11: Обязательства по операциям предоставления прав пользования
	стрк = ТЗ.Добавить();
	стрк.НомерСтроки = 11;
	стрк.НомерКолонки = 3;
	стрк.ТипПоказателяОтчета = Перечисления.хн_нфо_ТипПоказателяОтчета.ОстатокНаКонец;
...
Процедура СоздатьДетальныеПоказателиИзТЗ(ТЗ, Знач РежимТестирования = Ложь, Знач ВыводитьСообщения = Ложь)
	...
	Для Счетчик = 0 По ТЗ.Количество() - 1 Цикл
		...
		СоздатьИлиОбновитьПоказатель(
			Идентификатор,
			Идентификатор,
			Перечисления.хн_нфо_ВидыПоказателейОтчета.ПустаяСсылка(),
...
Функция ВычислитьПоказатель(НомерСтроки, НомерКолонки, ДатаОстатка, СтруктураПараметров, Знач ВыводитьСообщения = Ложь, Знач РежимТестирования = Ложь) Экспорт

	#region initialization

	мДатаОстатка = ДатаОстатка;
	мСтруктураПараметров = СтруктураПараметров;
	мВыводитьСообщения = ВыводитьСообщения;
	...
	Если НомерСтроки = 2 И НомерКолонки = 3 Тогда

		ЗначениеПоказателя = ВычислитьПоказатель_стр2кол3();

	ИначеЕсли НомерСтроки = 2 И НомерКолонки = 4 Тогда

		ЗначениеПоказателя = ВычислитьПоказатель_стр2кол4();

	Иначе

		// Остальные показатели пока возвращают 0 (требуют уточнения счетов)
		ЗначениеПоказателя = 0;

		Если ВыводитьСообщения Тогда
			Сообщить("<validation_result>Показатель стр" + Формат(НомерСтроки, "ЧГ=") + "кол" + Формат(НомерКолонки, "ЧГ=") + " пока не реализован</validation_result>");
		КонецЕсли;
...
Функция ВычислитьПоказатель_стр2кол3()
	...
	МассивСчетов.Добавить(ПланыСчетов.Хозрасчетный.НайтиПоКоду("71503331"));
	МассивСчетов.Добавить(ПланыСчетов.Хозрасчетный.НайтиПоКоду("71504421"));
	МассивСчетов.Добавить(ПланыСчетов.Хозрасчетный.НайтиПоКоду("71505341"));
	МассивСчетов.Добавить(ПланыСчетов.Хозрасчетный.НайтиПоКоду("71506431"));
	...
	Для Каждого СчетСсылка Из МассивСчетов Цикл
		...
		Если ЗначениеЗаполнено(СчетСсылка) Тогда
			...
			ОстатокСчета = ПолучитьОстатокСчета(мСтруктураПараметров.Организация, СчетСсылка, мДатаОстатка, Ложь);
			ЗначениеПоказателя = ЗначениеПоказателя + ОстатокСчета;

			Если мВыводитьСообщения Тогда
				Сообщить("<data_summary>Счет: " + СчетСсылка.Код + ", Остаток: " + Формат(ОстатокСчета, "ЧРД=.; ЧГ=0") + "</data_summary>");
			КонецЕсли;
	...
	Если мВыводитьСообщения Тогда
		Сообщить("<data_summary>Итого стр2кол3 (Заемные деньги на отчетную дату): " + Формат(ЗначениеПоказателя, "ЧРД=.; ЧГ=0") + "</data_summary>");
	КонецЕсли;
...
Функция ВычислитьПоказатель_стр2кол4()
	...
	МассивСчетов.Добавить(ПланыСчетов.Хозрасчетный.НайтиПоКоду("71503332"));
	МассивСчетов.Добавить(ПланыСчетов.Хозрасчетный.НайтиПоКоду("71504422"));
	МассивСчетов.Добавить(ПланыСчетов.Хозрасчетный.НайтиПоКоду("71505342"));
	МассивСчетов.Добавить(ПланыСчетов.Хозрасчетный.НайтиПоКоду("71506432"));
...
// Назначение: Получение остатка по счету бухгалтерского учета на указанную дату
// Параметры: Организация – СправочникСсылка.Организации – Организация;
//            СчетСсылка – ПланСчетовСсылка.Хозрасчетный – Счет учета;
//            ДатаОстатка – Дата – Дата расчета остатка;
//            ЭтоОстатокНаНачало – Булево – Признак расчета остатка на начало (иначе на конец)
// Возвращаемое: Число – Сумма остатка по счету
// Операции с БД: Запрос к регистру бухгалтерии
// Производительность: Выполняет один запрос к БД
Функция ПолучитьОстатокСчета(Организация, СчетСсылка, ДатаОстатка, ЭтоОстатокНаНачало = Ложь)
	...
	Если ЭтоОстатокНаНачало Тогда

		Запрос.Текст =
		"ВЫБРАТЬ
		|	СУММА(ХозрасчетныйОстатки.СуммаНачальныйОстатокКт) - СУММА(ХозрасчетныйОстатки.СуммаНачальныйОстатокДт) КАК СуммаОстаток
		|ИЗ
		|	РегистрБухгалтерии.Хозрасчетный.Остатки(&ДатаОстатка, Счет = &Счет, , Организация = &Организация) КАК ХозрасчетныйОстатки";

	Иначе

		Запрос.Текст =
		"ВЫБРАТЬ
		|	СУММА(ХозрасчетныйОстатки.СуммаКонечныйОстатокКт) - СУММА(ХозрасчетныйОстатки.СуммаКонечныйОстатокДт) КАК СуммаОстаток
		|ИЗ
		|	РегистрБухгалтерии.Хозрасчетный.Остатки(&ДатаОстатка, Счет = &Счет, , Организация = &Организация) КАК ХозрасчетныйОстатки";
